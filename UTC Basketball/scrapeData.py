...
def shot_quality_select_team(chosen_team):
    ...
    logo_image.screenshot("UTC Basketball\\img\\general_team_data\\"+chosen_team+"_logo_image.png")
...
def shot_quality_player_page(player_name, save_location):
    ...
    wait(1)
    #Scroll to item wanted to click
    driver.execute_script("arguments[0].scrollIntoView();", player_link)
    #now scroll up some to beat the weird element getting in the way
    ...
    try: #this should work unless the player name doesn't match the player name on the page
        ...
        wait(2)
        player_stats.screenshot(save_location+'\\shot_quality_player_stats.png')
        ...
        try: #if there are player_in_cards, capture them
            ...
            player_in_cards.screenshot(save_location+'\\shot_quality_player_in_cards.png')
        ...
        try: #if there is a player_stats_table, capture it
            ...
            player_stats_table.screenshot(save_location+'\\shot_quality_player_stats_table.png')
...
#Run this alone to update the pdf without running the program
def test_pdf(team_name):
    teamSearches.find_players(team_name)
    createPDF.createPDF(team_name)

def main(selected_team):
    #gathers player names from ShotQuality and FastScout
    teamSearches.find_players(selected_team)
    ...
    for i in range(len(teamSearches.shot_quality_collection)):
        name_chunk = teamSearches.shot_quality_collection[i][:5]
        tracker = save_location
        for j in range(len(teamSearches.fast_scout_collection)):
                if teamSearches.shot_quality_collection[i] in tracker_array[j] or name_chunk in tracker_array[j]:
        ...
        if save_location == tracker:
            os.mkdir("UTC Basketball\\img\\shot_quality\\"+teamSearches.shot_quality_collection[i]+"randID="+random.randint(1, 999))
    ...
    #Start of Bartorvik Stuff
    open_bartorvik(selected_team)
    capture_bartorvik_stats(selected_team)
    #End of Bartorvik Stuff
    #Create PDF
    createPDF.createPDF(selected_team)
    createTextlessPDF.createPDF(selected_team)
    print("I'm Done!")
